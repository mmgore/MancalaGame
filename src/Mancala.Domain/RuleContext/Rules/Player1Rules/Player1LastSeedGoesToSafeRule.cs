...
{
    public class Player1LastSeedGoesToSafeRule : IRule
    {
        ...
        private bool CheckIfPlayerLastSeedGoestoSafe(Board board, Player player)
            => (board.CurrentIndex == board.MoveLastIndex && player.PlayerType == PlayerType.Player1 && board.CurrentIndex == board.P1SafeIndex);
