...
{
    public class Player2LastSeedWentToEmptyHouseRule : IRule
    {
        ...
        private bool CheckIfPlayerLastSeedWenttoEmptyHouse(Board board)
            => board.CurrentIndex == board.MoveLastIndex && board.Houses[board.MoveLastIndex].Count == 0 && board.CurrentIndex != board.P2SafeIndex;

        private void SetSafeCountAccordingtoPlayertype(Board board)
        {
            int targetIndex = 12 - board.CurrentIndex;
            board.Houses[board.P2SafeIndex].Count = board.Houses[targetIndex].Count + 1;
